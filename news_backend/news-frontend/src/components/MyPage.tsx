...
const MyPage: React.FC<MyPageProps> = ({ user }) => {
    ...
    const fetchProfileData = async (isRetry = false) => {
        ...
        // Firebase 연결 상태 확인 (실패해도 계속 진행)
        const isConnected = await checkFirebaseConnection();
        if (!isConnected) {
            console.log('Firebase 연결 실패, 백엔드 데이터만 사용합니다.');
        }
        ...
        const timeoutId = setTimeout(() => {
            ...
            setError(
                '데이터를 가져오는데 시간이 오래 걸립니다. 다시 시도해주세요.'
            );
            setLoading(false);
            setIsRetrying(false);
        }, 15000);
        ...
        try {
            console.log(
                '마이페이지: 사용자 데이터 가져오기 시작',
                user.uid,
                isRetry ? '(재시도)' : ''
            );

            // 백엔드에서 사용자 데이터 가져오기 시도
            let backendUserData = null;
            try {
                const backendResponse = await fetch(
                    `http://127.0.0.1:8000/users/firebase/${user.uid}`
                );
                ...
                console.log(
                    '마이페이지: 백엔드 데이터 가져오기 실패, Firestore 사용'
                );
            }

            // Firestore에서도 데이터 가져오기 (오류 처리 포함)
            let userData = null;
            try {
                const userDoc = await getDoc(doc(db, 'users', user.uid));
                console.log(
                    '마이페이지: Firestore 문서 존재 여부',
                    userDoc.exists()
                );
                ...
                if (userDoc.exists()) {
                    ...
                    console.log(
                        '마이페이지: Firestore 사용자 데이터',
                        userData
                    );
                }
            } catch (firestoreError) {
                console.log(
                    '마이페이지: Firestore 데이터 가져오기 실패, 백엔드 데이터만 사용:',
                    firestoreError
                );
            }

            // 백엔드 데이터를 우선으로 하고, 없으면 Firestore 데이터 사용
            const finalUserData = backendUserData || userData;

            if (finalUserData) {
                setProfileData({
                    name:
                        finalUserData.display_name || finalUserData.name || '',
                    nickname: finalUserData.nickname || '',
                    gender: finalUserData.gender || '',
                    birthDate: finalUserData.birthDate || '',
                    politicalScore:
                        finalUserData.political_leaning_score ||
                        finalUserData.politicalScore ||
                        50,
                    activityScore: finalUserData.activityScore || 50,
                    profileImage:
                        finalUserData.profile_image_url ||
                        finalUserData.profileImage ||
                        '',
                });
                ...
                console.log('마이페이지: 사용자 데이터를 찾을 수 없음');
                setError(
                    '사용자 프로필을 찾을 수 없습니다. 회원가입을 다시 해주세요.'
                );
                clearTimeout(timeoutId);
            }
        } catch (error) {
            ...
            if (
                ...
                if (retryCount < 3) {
                    ...
                    setError(
                        '서버 연결에 실패했습니다. 네트워크 상태를 확인하고 새로고침해주세요.'
                    );
                }
            } else {
                setError(
                    `프로필 데이터를 가져오는데 실패했습니다: ${errorMessage}`
                );
            }
            clearTimeout(timeoutId);
        } finally {
    ...
    const handleSetActivityAsProfile = async () => {
        ...
        try {
            ...
            const newPoliticalAffiliation =
                getPoliticalAffiliationFromScore(newPoliticalScore);
